...
function LogoItem({ logo, size }: { logo: ClientLogo; size: "sm" | "md" }) {
  const dims = sizeMap[size];
  const inner = (
    <span
      className={cn(
        "relative inline-flex h-[var(--logo-h)] w-full items-center justify-center",
        "opacity-55 grayscale transition duration-300",
...
export function ClientLogoStrip({
  ...
  return (
    <div className={cn("w-full", className)}>
      {showLabel && (
        <p className="mb-3 font-mono text-[9px] uppercase tracking-[0.18em] text-ink-soft/80">
