...
type SpringboardClockfaceFaceProps = {
  now: Date;
  className?: string;
  gradientId?: string;
};
...
/** Analog clock dial SVG — shared by standalone and combined clock widgets. */
export function SpringboardClockfaceFace({
  now,
  className,
  gradientId = "springboard-clockface-bg",
}: SpringboardClockfaceFaceProps) {
  const hourDeg = handAngle(now, "hour");
  const minuteDeg = handAngle(now, "minute");

  return (
    <svg className={cn("springboard-clockface-svg", className)} viewBox="0 0 100 100" aria-hidden>
      <defs>
...
export function SpringboardClockfaceWidget({
  ...
  return (
    <div
      ...
      role="img"
    >
      <SpringboardClockfaceFace now={now} />
    </div>
  );
}
